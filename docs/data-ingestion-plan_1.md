...
| **Alpha Vantage** | `FundamentalsProvider` / `MarketDataProvider` (documented fallback only) | ~25 requests/day | Not actively scheduled — reserve for spot-checks or a single specific data point Alpaca/FMP can't supply |
| **FMP** | `FundamentalsProvider` (primary) + `MarketDataProvider` (secondary/cross-check) | 250 calls/day | Company fundamentals/ratios, EOD price cross-check |
...
| **SEC EDGAR** | `FilingsProvider` | Free, no key (requires a descriptive `User-Agent` header) | Filing metadata (8-K/10-Q/10-K/Form 4) + XBRL `companyfacts` for structured fundamentals |
