...
**Status check, 2026-09-13 — not yet built, and not yet actionable regardless:** only the single `price_fundamentals_macro` lineage exists in code (`app/ml/dataset.py`'s `FEATURE_SET_LABEL` is one hardcoded constant, not switched per lineage); there is no code path that trains a second, news-inclusive challenger, and nothing that automatically detects the §15 rollout threshold and turns one on. This gap is currently moot rather than urgent: the news pipeline's scheduler had never actually been running in this environment until fixed the same day this note was added (see `[[news_pipeline_scheduler_never_ran]]` memory) — real news coverage is on the order of days deep, nowhere near the 80%-of-60-trading-days bar this section's own §15 cross-reference requires before a news-inclusive lineage would even be trustworthy to train. Building the second lineage is real, undone engineering work for whenever that bar is actually within reach — don't infer it exists from this section reading "finalized."
...
**Amended 2026-09-10, evidence-based (spec §27: data quality over following a plan the evidence has since contradicted):** two things changed after real implementation and testing, both recorded in `app/ml/train.py`.

First, a real bug in the formula actually shipped initially: `weight = exp(-age_in_days / halflife)` decays to `1/e ≈ 0.368` at one halflife, not `0.5` — an e-folding time constant, not an actual half-life, contradicting both the name and this section's own stated "~25% at 2 half-lives, ~12% at 3" (`0.5**2=0.25`, `0.5**3=0.125` match; `exp(-2)=0.135`, `exp(-3)=0.050` do not). Fixed to the actual half-life form, `weight = 0.5 ** (age_in_days / halflife)`.

Second, and more consequential: `USE_RECENCY_WEIGHTING_DEFAULT` is currently **`False`** (flat weighting is the real default), not the recency-weighted scheme this section describes as finalized. This was decided by a real A/B comparison run against the live ~2-year dataset (`compare_weighting_schemes`, see `[[recency_weighting_ab_test]]` memory) — flat weighting outperformed 252-day-halflife recency weighting on real backtest metrics at the ~2-year data depth available at the time. The halflife constant (252 days) and the corrected formula both remain implemented and available (`compute_sample_weights`, `RECENCY_HALFLIFE_DAYS`), and every trained model still records its actual weighting scheme in `model_versions.hyperparameters` — so this is a default flip, not a removal, and directly revisitable: re-run the comparison as the dataset grows past ~3 years (this section's own "effective operative window" reasoning implies recency-weighting's advantage should become clearer at that depth, since flat-weighting's staleness problem gets worse precisely as history accumulates). Flip `USE_RECENCY_WEIGHTING_DEFAULT` back to `True` only once recency-weighting is clearly and consistently better than flat on real evidence, not on the a priori reasoning above alone.
...
**"News" page — news picks: a second daily top-5 on the ranking model's exact cycle, ranked only by news (redesigned 2026-09-19; replaces the earlier rolling headline digest):** a separate public page (`/news`) shows, per trading day, the five stocks with the most net-positive classified news, locked once at ~09:15 ET (the same minute as `prediction_generation`, Mon–Fri) and never modified afterwards (tables `news_pick_runs` / `news_picks` / `news_pick_results`, mirroring `prediction_runs` / `predictions` / `prediction_results`). Selection is point-in-time: only articles both **published and classified** at or before the run's cutoff (`as_of`) count, from the previous regular session's real ~16:00 ET close (from the benchmark's own trading calendar; 24h if none is on record, never more than 5 days) up to the cutoff. Score per S&P 100 stock = sum over its window headlines of `sentiment × importance × relevance` (§5's LLM output); only net-**positive** stocks are ever picked, so fewer than five picks is an honest outcome on a quiet-news day, and a headline count/positive/negative breakdown plus the top-3 supporting headlines are frozen with each pick. This is deliberately **not** gated behind the 60-trading-day news-rollout bar above — it is not a training input to the ranking model and never touches `train_model`/`generate_predictions`. After the close (`result_evaluation`), each pick's realized return and the S&P 500's are written once (`compute_realized_label`); a pick is a hit when it **beat the S&P 500** that session — the same yardstick as the ranking model's `direction_correct` — and both raw returns are shown side by side. Only headline titles, source names and links are shown, never article bodies. Sentiment badges use neutral Positive/Negative/Neutral wording (§23) rather than Bullish/Bearish. A session whose lock time already passed can be **reconstructed** (`python -m app.jobs.generate_news_picks --date YYYY-MM-DD`): selection still uses only news classified by that session's 09:15 ET cutoff, and the row records that it was written later (`generated_at` vs `as_of`), surfaced in the UI as a "Reconstructed" label. The tabs are "Latest Picks" and "Last 7 Days"; the page's tiles are the 7-day hit rate, average excess return and picks graded (same shape as the ranking page's tiles).

**Schedule correction found while building this (2026-09-19):** `feature_generation` and `prediction_generation` previously fired every day including Saturday and Sunday, which would lock a never-evaluable snapshot for a day the market isn't open. All three morning lock jobs (`feature_generation`, `prediction_generation`, `news_pick_generation`) now run Mon–Fri only. Market holidays are not filtered (no holiday calendar is kept) and simply stay pending.
