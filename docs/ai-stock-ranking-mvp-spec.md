...
**"News" page — a same-day news digest, independent of the ranking model (added 2026-09-17):** a separate public page (`/news`) surfaces whichever classified headlines the LLM itself scored as most relevant + important, in the trailing 48 hours, ranked by `relevance × importance` — a per-headline, per-company display of `app/services/news_service.py`'s `get_top_news`. This is deliberately **not** gated behind the 60-trading-day rollout bar above, because it is not a training input to the ranking model and never touches `train_model`/`generate_predictions` — it is a direct, same-day display of the LLM's own classification output (§5), not a prediction. Only the headline title, source name, and a link back to the original article are shown (never the article body — `news_articles` doesn't store one), which is standard news-aggregator practice. Sentiment is shown as a neutral "Positive/Negative/Neutral" label rather than "Bullish/Bearish" — a deliberate wording choice (§23) to avoid reading as a directional trading call, since this is describing one headline's tone, not the platform's own recommendation. The page carries its own disclaimer distinguishing it from the AI Score ranking, so the two are never confused as two competing predictions.

**News page "Last 7 Days" tab (added 2026-09-18):** the same page also offers a second tab, browsing the trailing 7 calendar days (excluding today, which the first tab already covers) bucketed by day — `get_news_history`, top 5 scored items per day. Purely a browse/history view of the same independent digest; still never touches the ranking model or its training data.
